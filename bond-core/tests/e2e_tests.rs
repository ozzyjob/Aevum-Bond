...
fn test_cli_help_commands() {
    ...
    if binary_exists(&bond_cli) {
        let output = Command::new(&bond_cli).arg("--help").output();
    ...
    if binary_exists(&aevum_cli) {
        let output = Command::new(&aevum_cli).arg("--help").output();
    ...
    if binary_exists(&wallet_cli) {
        let output = Command::new(&wallet_cli).arg("--help").output();
    ...
    assert!(
        successful_tests > 0 || !binary_exists(&bond_cli),
        "At least one CLI help command should work if binaries exist"
    );
}
...
fn test_cli_version_commands() {
    ...
    if binary_exists(&bond_cli) {
        let output = Command::new(&bond_cli).arg("--version").output();
    ...
    if binary_exists(&aevum_cli) {
        let output = Command::new(&aevum_cli).arg("--version").output();
    ...
    if binary_exists(&wallet_cli) {
        let output = Command::new(&wallet_cli).arg("--version").output();
    ...
    assert!(
        successful_tests > 0 || !binary_exists(&bond_cli),
        "At least one CLI version command should work if binaries exist"
    );
}
...
fn test_cli_error_handling() {
    ...
    if binary_exists(&bond_cli) {
        let output = Command::new(&bond_cli).arg("invalid-command").output();
    ...
    if binary_exists(&aevum_cli) {
        let output = Command::new(&aevum_cli).arg("invalid-command").output();
    ...
    if binary_exists(&wallet_cli) {
        let output = Command::new(&wallet_cli).arg("invalid-command").output();
    ...
    println!(
        "  ✅ Graceful Failures: {}/{}",
        graceful_failures, total_tests
    );
    println!(
        "  🎯 Error Handling Rate: {:.1}%",
        (graceful_failures as f64 / total_tests as f64) * 100.0
    );

    // At least one CLI should handle errors gracefully
    assert!(
        graceful_failures > 0 || !binary_exists(&bond_cli),
        "At least one CLI should handle errors gracefully if binaries exist"
    );
}
...
fn test_basic_cli_functionality() {
    ...
    println!(
        "  🎯 Functionality Rate: {:.1}%",
        (functional_tests as f64 / total_tests as f64) * 100.0
    );

    // At least some functionality should work
    assert!(
        functional_tests >= 0,
        "CLI functionality tests should complete"
    );
}
