...
fn test_block_creation_and_validation_integration() {
    ...
    assert_eq!(test_block.transactions.len(), 1, "Block should have 1 transaction");
...
fn test_transaction_creation_and_validation_integration() {
    ...
    assert_eq!(coinbase_tx.outputs.len(), 1, "Coinbase transaction should have 1 output");
    
    // Test transaction hash calculation
    let tx_hash = coinbase_tx.hash();
    assert!(tx_hash.is_ok(), "Transaction hash calculation should succeed");
    
    // Test transaction validation
    let validation_result = coinbase_tx.validate();
    assert!(validation_result.is_ok(), "Coinbase transaction validation should succeed");
    
    // Test total output value calculation
    let total_output = coinbase_tx.total_output_value();
    assert!(total_output.is_ok(), "Total output calculation should succeed");
    assert_eq!(total_output.unwrap(), 5_000_000_000, "Total output should match coinbase amount");
}
...
fn test_utxo_creation_and_management_integration() {
    ...
    assert!(utxo_display.len() > 0, "UTXO ID should have a display format");
    
    // Test programmable UTXO creation
    let script = Script { code: vec![0x63, 0x64, 0x68] }; // Simple script
    ...
    assert_eq!(programmable_utxo.value, 1_000_000_000, "UTXO value should match");
...
fn test_script_interpreter_integration() {
    ...
    assert!(execution_result.is_ok(), "Simple script execution should succeed");
}
...
fn test_consensus_validation_integration() {
    ...
    assert!(validation_result.is_err(), "Block with invalid PoW should fail validation");
    
    // Test difficulty calculation
    let current_target = DifficultyTarget::MAX;
    let difficulty_result = consensus.calculate_next_difficulty(&[], current_target);
    assert!(difficulty_result.is_ok(), "Difficulty calculation should succeed");
}
...
fn test_complete_block_lifecycle_integration() {
    ...
    assert!(final_block.header.validates_pow().unwrap(), "Mined block should have valid PoW");
    ...
    assert!(final_block.hash().is_ok(), "Final block should hash successfully");
    assert_eq!(final_block.transactions.len(), 1, "Block should contain the transaction");
