...
fn test_script_utxo_integration() {
    ...
    let programmable_utxo = ProgrammableUtxo::new(utxo_id, 1_000_000_000, script.clone(), 0);
    ...
    assert_eq!(
        ...
        "UTXO value should be preserved"
    );
...
fn test_complete_blockchain_simulation() {
    ...
    for i in 1..=3 {
        ...
        // Create mining header
        let header =
            mining::create_mining_header(i, previous_hash, merkle_root, DifficultyTarget::MAX);
        ...
        assert_eq!(
            new_block.header.previous_hash, previous_hash,
...
fn test_error_propagation_integration() {
    ...
    assert!(
        pow_result.is_ok() || pow_result.is_err(),
